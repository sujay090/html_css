...
let fromvalue=document.querySelector("select");
let tovalue=document.querySelector("select");
...
for(let select of Countrylist){
  for(contryCode in countryList){
    let newOptain=document.createElement("option")
    newOptain.textContent=contryCode;
    newOptain.value=contryCode;
    // if(select.name ==="form" && countryCode ==="USD"){
    //   newOptain.selected="selected"
    // }else if(select.name ==="to" && countryCode ==="INR"){
    //   newOptain.selected="selected"
    // }

    select.appendChild(newOptain)
